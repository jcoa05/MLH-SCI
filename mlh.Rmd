...
cat(sprintf("Shap worker Count:  %d\n", shap_workers))
...
if (file.exists(MEMORY_LOG_FILE)) {
  ...
  cat("------------------------------------------------------------------\n")
  cat(sprintf("4GB Feasibility Threshold: %s\n", 
              ifelse(global_peak_rss_gb < 4, "PASSED", "FAILED")))
  cat(sprintf("Headroom: %.2f GB (%.1f%% of 4GB limit)\n",
              4 - global_peak_rss_gb, 
              100 * (4 - global_peak_rss_gb) / 4))
  cat("==================================================================\n\n")
  ...
  feasibility_result <- list(
    ...
    passed_4gb = global_peak_rss_gb < 4
  )
  ...
  memory_plot <- memory_timeline %>%
    ...
    annotate("text", x = max(memory_timeline$elapsed_sec/60) * 0.02, y = 8.3, 
             label = "4GB Feasibility Threshold", hjust = 0, color = "red", 
    ...
    labs(
      ...
      subtitle = sprintf("Peak RSS: %.2f GB | Runtime: %.1f min | 4GB Threshold: %s",
...
cat(sprintf("   - Peak physical RAM (RSS): %.2f GB\n", peak_memory_gb))
cat(sprintf("   - 4GB feasibility threshold: %s\n",
            ifelse(peak_memory_gb < 4, "PASSED", "FAILED")))
...
cat("                         CONCLUSION                              \n")
cat("==================================================================\n")
if (peak_memory_gb < 4 && total_time_hours < 8) {
  cat("All workflow components completed successfully within the defined\n")
  cat("resource constraints (4GB RAM, overnight execution). This demonstrates\n")
  ...
  cat(sprintf("   - RAM: %.2f GB (threshold: 4GB) - %s\n", 
              peak_memory_gb, ifelse(peak_memory_gb < 4, "OK", "EXCEEDED")))
...
cat(sprintf("UNDER_4GB_THRESHOLD: %s\n", ifelse(peak_rss < 4, "TRUE", "FALSE")))
...
cat(sprintf("OVERALL_FEASIBLE: %s\n", ifelse(peak_rss < 4 && total_time_hours < 8, "TRUE", "FALSE")))
...
# PHASE 10: EVALUATION PLOTS {.tabset}
...
RECOVERY_THRESHOLD <- PARAMS$recovery_thresh
