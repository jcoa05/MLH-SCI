...
# We temporarily throttle the worker count to a safe number (2).
# This provides ~2x speedup without the memory risk of using all cores.
safe_shap_workers <- 2 
cat(sprintf("Throttling parallel workers to %d for memory safety during SHAP...\n", 
            safe_shap_workers))

# Re-plan for this specific heavy task
# This uses your existing doFuture setup but limits the active sessions.
original_plan <- plan() # Save original plan
plan(multisession, workers = safe_shap_workers) 
